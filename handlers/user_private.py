...
user_level_3_ids = [2119353166, 5850783737, 7681168842, 1139034211, 7877529350, 7877529350, 7777781328, 6091002843, 5255725303, 1186939115, 5276190685, 6804661582, 6668604221, 6126719516, 5828167970, 6283356137, 6662455423, 2097858255, 7698243378, 6672279908, 6601893986, 6209517485, 6032405861, 6113872493, 5776038271, 6710467312]
my_ids = [2119353166]
user_level_1_ids = [2119353166, 8156886610]#, 2119353166, 6627798006, 7294169822, 6356510135, 5879786452, 7968414403, 7394276254, 7740277351, 6483117741, 6283614567, 984742231, 1451398913, 5569265124, 6934691008, 5977196372, 5534205178, 7450839606, 771827523, 6528529601, 7331880668, 6593272248, 7876684402]
...
async def welcome2homework(message):
    ...
    scheduler.add_job(send_homework_for_beginners, 'cron', hour=6, minute=0, kwargs={"message": message})
    scheduler.start()
...
def schedule_daily_tests(bot: Bot):
    ...
    scheduler.start()
    scheduler.add_job(start_new_test_for_all, "cron", hour="1", minute=10, args=[bot])
    scheduler.add_job(send_homework_for_beginners, "cron", hour="1", minute=15, args=[bot])
